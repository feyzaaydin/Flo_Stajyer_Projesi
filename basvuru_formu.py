...
def cv_analiz_ve_dogrulama_yap(cv_metni, yetkinlikler, form_ad_soyad, form_bolum, form_universite, form_sinif, key):
    """CV'den yetkinlik çıkarma ve form bilgileriyle tutarlılık kontrolünü TEK bir
    AI çağrısında birleştirir. Önceden bunlar iki ayrı çağrıydı ve bekleme süresini
    ciddi şekilde uzatıyordu; artık tek istek olduğu için ~yarı yarıya hızlanır."""
    client = genai.Client(api_key=key)
    prompt = f"""Sen hem bir IT/İK işe alım uzmanı hem de bir CV doğrulama uzmanısın. Aşağıdaki iki görevi
    TEK seferde, aynı CV metnini kullanarak yap.

    GÖREV 1 - YETKİNLİK ANALİZİ:
    Adayın CV'sini derinlemesine analiz et. Sadece şu havuzdaki kelimeleri kullanarak eşleşenleri bul:
    {', '.join(yetkinlikler)}

    GÖREV 2 - TUTARLILIK KONTROLÜ:
    Aday başvuru formuna şu bilgileri girdi:
    ...
    YETKINLIKLER: <sadece eşleşen yetkinlikleri virgülle ayırarak yaz>
    ...
    for deneme in range(3):
        try:
            ...
            ai_yetkinlikler = [y.strip() for y in sonuc.get("YETKINLIKLER", "").split(",") if y.strip()]
            tutarlilik_notu = sonuc.get("ACIKLAMA", "").strip() if sonuc.get("TUTARLI", "").upper().startswith("HAY") else ""
